...
export async function POST(request: NextRequest) {
  try {
    ...
    const { apiKey, payload, stream } = body;
    ...
    if (contentType?.includes("application/json")) {
      try {
        ...
        return NextResponse.json(
          { error: `HTTP ${response.status}: Invalid JSON response from Groq API` },
      ...
      return NextResponse.json(
        { error: `HTTP ${response.status}: ${textResponse || "Unknown error from Groq API"}` },
        { status: response.status }
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Proxy request to Groq API for model list
